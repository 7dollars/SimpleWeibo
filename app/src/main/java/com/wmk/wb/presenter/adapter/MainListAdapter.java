...
public class MainListAdapter extends RecyclerView.Adapter  {
    ...
    public MainListAdapter(Context context, SwipeRefreshLayout swipe,Subscriber<DetialsInfo> s,Subscriber<String> s2) {
        super();
        mContext = context;
        mswipe = swipe;
        mSubscriber=s;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch(getItemViewType(position))
        {
            ...
            /*    if (CacheUtil.getBitmapFromMemCache(StaticData.getInstance().data.get(i).getHeadurl()) != null) {
                    holder3.head.setImageBitmap(CacheUtil.getBitmapFromMemCache(StaticData.getInstance().data.get(i).getHeadurl()));
                }
                else
                {
                    String url=StaticData.getInstance().data.get(i).getHeadurl();
                    DataManager.getInstance().getPic(loadHead(url,i,this), url);
                }*/
            ...
            case 1:
            {
             ...
             /*   if (CacheUtil.getBitmapFromMemCache(StaticData.getInstance().data.get(i).getHeadurl()) != null) {
                ...
                Glide.with(mContext).load(data.getHeadurl()).into(holder3.head);
            /*    if(CacheUtil.getBitmapFromMemCache(StaticData.getInstance().data.get(i).getRet_headurl()) == null)
                {
                    String url=StaticData.getInstance().data.get(i).getRet_headurl();
                    DataManager.getInstance().getPic(loadHead(url,i,this), url);
                }*/
